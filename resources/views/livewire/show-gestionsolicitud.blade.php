                    ...
                    <x-slot name="bodytable">
                        @foreach ($consulta as $item)
                            <x-tr x-data="{ openOption: false }">
                                ...
                                <x-td wire:click="ver('{{ $item->id }}')" class="uppercase">{{ $item->usuario->documento ?? '' }}
                                </x-td>
                                <x-td wire:click="ver('{{ $item->id }}')" class="uppercase">{{ $item->usuario->name ?? '' }}
                                </x-td>
        ...
        <x-modal-crud x-show="ver">
            ...
            @if (!empty($consultaVer))
                ...
                <x-slot name="botones">
                    ...
                    <x-primary-button x-on:click="ver = false" class="bg-red-400" wire:click="cancelar('{{ $consultaVer->id }}')">
                        Cancelar
                    </x-primary-button>
                    <x-primary-button x-on:click="ver = false" wire:click="aprobar('{{ $consultaVer->id }}')">
                        Aprobar
                    </x-primary-button>
                </x-slot>
            @else
