                                ...
                                <x-td wire:click="ver('{{ $item->id }}')">
                                    ...
                                    @if (!empty($item->modulos))
                                        <div class="flex flex-wrap space-x-1 min-w-fit">
                                            ...
                                            @foreach ($item->modulos as $itemTabla)
                                                <div
        ...
        <x-modal-crud x-show="nuevo">
            ...
            <x-slot name="campos">
                ...
                <div class="space-x-2 mb-3">
                    <label class="block text-gray-700  text-sm font-semibold mb-2">Directivas</label>
                    <x-checkbox-with-label type="checkbox" id="tipo" name="tipo" value="" wire="rol_super"
                        label="Rol Global" />
                    <x-checkbox-with-label type="checkbox" id="tipo" name="tipo" value=""
                        wire="rol_administrador" label="Rol Empresarial" />
                </div>


                <div class="block text-gray-700 text-sm font-semibold mb-2 ">Modulos</div>
                        ...
                        <div class="w-full border rounded border-gray-200">
                            @foreach ($consultaModulos as $itemNuevo)
                                <div class="text-sm cursor-pointer hover:bg-gray-100 w-full py-1 px-2 border-b border-gray-100 "
                                    wire:click="selectModulo('{{ $itemNuevo }}')">{{ $itemNuevo->nombre }}</div>
                            @endforeach
                        </div>
                    </div>
                </div>
                    ...
                    @if (!empty($modulosVer))
                        <div class="flex flex-wrap space-x-1 w-56">
                            ...
                            @foreach ($modulosVer as $itemVer)
                                <div
                                    class="rounded bg-blue-200 text-blue-600 my-1 px-2 py-1 text-sm flex align-middle capitalize">
                                    {{ $itemVer->nombre }}</div>
                            @endforeach
